...
class JCM(nn.Module):
    def __init__(self, config, device):
        ...
        self.encoder = Encoder(self.config,prototype=None)
        ...
        self.decoder_recon = Decoder_Recon(self.config,prototype=None)

        if self.config.mod_method == 'bpsk':
            self.decoder_class = Decoder_Class(int(config.channel_use / 2), int(config.channel_use / 8),config=self.config,prototype=None)
        else:
            self.decoder_class = Decoder_Class(int(config.channel_use * 2 / 2), int(config.channel_use * 2 / 8),config=self.config,prototype=None)
